...
3. Firebase CLI: `npx firebase-tools@latest login` must be run by the founder in a real terminal — `--no-localhost` needs the CLI process to stay alive to register the session, which this harness cannot do. Once logged in, the credentials sit in the user profile and every CLI call from a session works.
...
5. Then Dependabot triage (11 PRs). The Prettier debt and the `format:check` gate are done (CB-087, #49). Next free backlog id: CB-089.
