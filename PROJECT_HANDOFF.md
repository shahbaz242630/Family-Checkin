...
2. Sprint 4 wave 2 = the Phase 3 rows that need the founder's accounts, which now exist. In order: CB-027 (EAS project id and owner, `eas env:create` to replace the `${VAR}` interpolation, `versionCode`/`buildNumber`), then a development build, which is the only way CB-028 (social login), CB-078 (PKCE downgrade) and CB-082 (notifications chunk) can be verified at all, then CB-031 (FCM) and the device half of CB-030. Read `docs/providers/store-and-push-keys.md` first: it says which values are public config and which never enter the repo.
3. Two of those need the founder before code: converting `app.json` to `app.config.js` (a protected file, needs explicit approval) so `googleServicesFile` can read an EAS file secret, and running `npx eas init` to produce the project id.
...
5. Then Dependabot triage (11 PRs), the one-off Prettier formatting commit and the `format:check` CI gate — the formatting debt has now cost work in three consecutive waves. Next free backlog id: CB-087.
