...
- Sprint 1 (PRs #17 to #21), sprint 2 (#25 to #31), sprint 3 (#34 to #40, #42) and sprint 4 wave 1 (#43 to #47) are merged. Phase 1 is complete except CB-078. Phase 2 is complete in code; CB-022's audio waits on decision 3 and CB-083 is open. Phase 3 is nine rows further on: CB-029, CB-032 to CB-037, CB-039 and CB-041 are done. CB-027 is part done (#49): the app is configured for a store build but no build has been run, so its Done-when line is unobserved. `master` CI is green at `5da31e7`.
- Tests: 96 spec files, 1005 tests across all projects; the compiled build boots in fake and configured mode. Acceptance: `docs/audits/2026-09-07/sprint3-wave2-acceptance.md` and `docs/audits/2026-09-07/sprint4-wave1-acceptance.md`.
...
1. On `master`: `git status --short --branch`, `npm ci` (the lockfile gained `jose` in #35 and `zod` in #40), then `npm run verify` (and `npm run test:coverage` — `verify` alone does not exercise the thresholds CI enforces).
2. Sprint 4 wave 2 continues. CB-027's config half landed in #49; the next step is **a development build** (`eas build --profile development --platform android`), which is the only way CB-028 (social login), CB-078 (PKCE downgrade) and CB-082 (notifications chunk) can be verified at all, then CB-031 (FCM) and the device half of CB-030. A cloud build spends the founder's EAS quota and takes ~15 minutes — ask first. Read `docs/providers/store-and-push-keys.md`: it says which values are public config and which never enter the repo.
3. CB-031 needs the founder before code: a Firebase project, then `google-services.json` uploaded as a file-type EAS environment variable. The `app.config.js` conversion that makes it readable is already done; the `googleServicesFile` line is deliberately absent until the file exists, because pointing at a missing file fails the Android build.
4. Still open and blocking nothing: decision 3 (voice languages), decision 9 (where Terms/Privacy are hosted — the last part of CB-034), the WhatsApp templates or CB-083, and `OPERATIONS_PUSH_RECEIPTS_RUN_URL` when the backend is hosted.
5. Then Dependabot triage (11 PRs). The Prettier debt and the `format:check` gate are done (CB-087, #49). Next free backlog id: CB-088.
