...
- 12 Dependabot PRs are open, deferred.
...
2. **Sprint 4 wave 3 is closed.** Merged: #55 (CB-038 siren, the multer allowlist, the ci.yml sync), #56 (CB-083 WhatsApp template script) and #58 (the WhatsApp npm alias, wave 3 PR numbers). Phase 3 now has only these open: **CB-028, CB-030 device half, CB-034's Terms/Privacy (decision 9), CB-040.**

3. **Start here: one emulator boot closes four things.** Everything left in Phase 3 except CB-034 and CB-040 needs a device, and the debt has been accumulating since 2026-09-06 — so plan a single device session rather than four visits. Ask the founder before booting the emulator: they render video on this machine. Boot with the block in step 4, then in this order:

   1. **Settle the CB-028 test surface first — it costs one command.** `adb shell pm list packages | grep -i "chrome\|browser"`. If a browser is present, CB-028 is testable on the AVD that already exists and **no 1.5 GB Play Store image is needed**; if not, install a browser APK before considering a new image. The system WebView is **not** a browser and does not provide Custom Tabs.
   2. **CB-028 itself.** The blocker recorded before 2026-09-09 ("needs a Play Store image or a real phone") is probably false: `signInWithGoogle` (`apps/mobile/src/services/auth.ts:116`) uses `supabase.auth.signInWithOAuth` plus `WebBrowser.openAuthSessionAsync` — a Chrome Custom Tab **web** flow needing a browser, not Play Services. The bug is traced: `auth.ts:113` generates a custom `state` into SecureStore and passes it via `queryParams`, but Supabase's PKCE flow manages its own `state`, so ours never round-trips and `supabase.ts:108` rejects the callback on `expectedState !== stateParam`. Drop the custom `state` and its SecureStore expectation; the PKCE `code` exchange already provides the protection. **The identical pattern is in `signInWithApple` (`auth.ts:170`)** — a two-function fix, and Apple sign-in is presumably broken the same way, untested because no iOS build has ever been attempted. `auth.ts` and `supabase.ts` are **protected files: ask for a narrow, per-task approval before editing, and give a per-file account of every change in the PR.**
   3. **CB-030's device half** — tapping an escalation push opens `/(main)/receivers/{id}`, and after sign-out the backend `device_tokens` row is `active:false`.
   4. **Hear the new siren.** CB-038 shipped a 10.000 s / 44.1 kHz wail whose properties were verified by parsing the file — which is not the same as a person in a noisy room noticing it. This device pass owes an answer on whether it is loud and long enough, and is the moment to reconsider `SIREN_SPEC` in `apps/mobile/scripts/generate-escalation-siren.mjs`.

   Also owed on a device since sprint 4 wave 1, which has never had a device pass: cold-start email confirmation, the warm-start recovery link, a non-admin's drawer, the siren with the app backgrounded, and a sandbox purchase.
...
5. **Before any store build**: CB-038 is done in code but unheard on a device (step 3.4), and iOS still needs the Apple Team ID plus an APNs key on EAS, neither of which exists. No iOS build has ever been attempted.

6. **Still open and blocking nothing**: decision 3 (voice languages, blocking CB-022's audio), decision 9 (Terms/Privacy hosting — the founder is working on a domain), CB-040 (`expo-doctor` 15/18; keep it out of a parallel wave because it touches `package-lock.json`), and `OPERATIONS_PUSH_RECEIPTS_RUN_URL` when the backend is hosted.

   **CB-083 is built but not proven.** The script creates all 112 WhatsApp templates and emits `TWILIO_WHATSAPP_CONTENT_SIDS`, and is exercised only against a fake Content API — **no request has ever reached Twilio**, so the row stays open until it runs on a real account. When credentials arrive: `npm --prefix apps/backend run providers:create-whatsapp-templates -- --dry-run` first, then `--templates=consent_request --languages=en` for one, then all 112. It needs `npm run build` first (it loads the numbering helper from `dist/`). Two things to review before submitting to Meta: non-English copy is unreviewed machine translation, and button titles are the Latin keyword (`YES`, `STOP`) in every language because the reply parser only reads Latin — `docs/providers/whatsapp.md` §2 still says "titles in that language" and disagrees with the code.

7. Then Dependabot triage (12 PRs). Next free backlog id: **CB-091**.
