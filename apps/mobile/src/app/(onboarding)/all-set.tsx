...
export default function AllSetScreen() {
  ...
  console.log('AllSetScreen - authUser:', authUser?.id);
  console.log('AllSetScreen - onboarding data:', JSON.stringify(data, null, 2));
  ...
  const handleStartCheckins = async () => {
    console.log('handleStartCheckins called');
    setIsLoading(true);

    try {
      // Get user directly from Supabase auth
      const { data: { user }, error: userError } = await supabase.auth.getUser();
      console.log('Got user from Supabase:', user?.id, 'error:', userError);
      ...
      if (profileError) {
        console.error('Step 1 failed:', profileError);
        throw profileError;
      }
      ...
      if (lovedOneError) {
        console.error('Step 2 failed:', lovedOneError);
        throw lovedOneError;
      }
      console.log('Step 2 completed - Loved one created:', lovedOne.id);
      ...
      if (relationshipError) {
        console.error('Step 3 failed:', relationshipError);
        throw relationshipError;
      }
      console.log('Step 3 completed - Relationship created:', relationship.id);
      ...
      if (scheduleError) {
        console.error('Step 4 failed:', scheduleError);
        throw scheduleError;
      }
      ...
      if (escalationError) {
        console.error('Step 5 failed:', escalationError);
        throw escalationError;
      }
      console.log('Step 5 completed - Escalation plan created');
      ...
      if (
        ...
        const { error: contactError } = await supabase.from('contact_points').insert({
          owner_user_id: user.id,
        ...
        if (contactError) {
          console.error('Step 6 failed:', contactError);
          throw contactError;
        }
        console.log('Step 6 completed - Emergency contact created');
      } else {
        console.log('Step 6 skipped - No emergency contact needed');
      }
      ...
      if (subscriptionError) {
        console.error('Step 7 failed:', subscriptionError);
        throw subscriptionError;
      }
      console.log('Step 7 completed - Subscription created');

      // Reset onboarding context and navigate to main app
      console.log('All steps completed! Navigating to main app...');
      resetOnboarding();
      router.replace('/(main)');
    } catch (error) {
