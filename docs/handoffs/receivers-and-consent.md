...
| Layer   | Paths                                                                                                                                                                                                                                                                                                     |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Backend | `packages/shared-types/validation/receivers.ts` (the zod body schemas the routes validate against, CB-042); `apps/backend/src/modules/receivers/` — `receivers.controller.ts`, `receivers.service.ts`, `receiver-consent.service.ts`, `receiver-reply.service.ts`, `receiver-replies.controller.ts`, `receiver-replies.module.ts`, `prisma-receivers.repository.ts`, `abuse-review-pause.ts`, `receiver-policy.ts` (cooldown/resend/retry constants and the typed 409/429 errors); quiet pushes via `apps/backend/src/modules/notifications/notifications.service.ts` (`sendQuietUpdateToUser`) |
...
| Data    | `receivers` (incl. `consentResendCount`), `opt_out_cooldowns`, `abuse_reports`, `audit_logs`; migrations `202604260001_initial_nearby_schema`, `202605150001_receiver_remove_step_up`, `202609060301_receiver_consent_resend_count`                                                                                                                                                   |
...
All `/receivers` routes require a Supabase bearer token (`Authorization: Bearer <supabase access token>`), verified by `SupabaseAuthService`; the sender row is read (or inserted once) from the identity on every call through `UsersService.findOrCreateFromSupabaseIdentity` (CB-084), and every query is scoped by `userId` + `deletedAt: null`.
...
6. `POST /receivers/<id>/consent/resend` with the Supabase bearer on a fresh `PENDING` receiver → `429 CONSENT_RESEND_LIMIT` with `nextAllowedAt` = the first send + 24 h; on a receiver whose create returned `consentRequestStatus: "failed"` → 200 and the fake consent message (that counts as the first invitation). After a successful resend the next window is 7 days (`consentResendAllowedAt` on the response and on `GET /receivers/<id>`).
...
- `requestConsent` throws when `consentRequestedAt` is already set; only `resendConsent` may send again, and only for a `PENDING` receiver inside the window `consentResendAllowedAt` computes in `receiver-policy.ts`: `consentRequestedAt` + 24 h while `consentResendCount` is 0 (`CONSENT_FIRST_RESEND_DELAY_HOURS`), + 7 days once a resend happened (`CONSENT_RESEND_MIN_INTERVAL_DAYS`). The counter is incremented in the database by `markConsentRequested({ resend: true })`, and only when an invitation had actually gone out: a resend after a failed first send is the first invitation. A provider failure in either path never sets `consentRequestedAt` or the counter, so a receiver who never got the message can always be re-invited.
...
- (#PR) (sprint 3 wave 2): CB-042 zod body schemas for create, update, pause, resolve and the fake reply route, applied through `ZodBodyPipe`; receiver field failures carry `RECEIVER_FIELD_INVALID`. CB-084 the `upsertFromSupabaseIdentity` alias is gone; the controller calls `findOrCreateFromSupabaseIdentity`.
