...
- `POST /billing/revenuecat/webhook` — RevenueCat only, no user session. Auth header: `Authorization`, falling back to `x-revenuecat-authorization`; a `Bearer ` prefix is stripped and the remainder compared in constant time (`isMatchingSecret`, `shared/auth/bearer-secret.ts`) to `REVENUECAT_WEBHOOK_AUTH_TOKEN`. Missing token, missing config or mismatch → `401`. Auth is checked first; then `revenueCatWebhookBodySchema` (`@nearby/shared-types`) checks the shape — `event` an object, `purchased_at_ms` a number, `entitlement_ids` an array of strings — and `parseRevenueCatEvent` still decides which fields an event must carry: missing or mistyped `type`, `id`, `app_user_id`, `product_id`, `transaction_id` (non-empty strings) or `purchased_at_ms` (finite number > 0) → `400` (CB-026, CB-042). The schema is deliberately permissive about *which* fields are present so a store that adds one to an event we already accept does not start failing. Non-string `entitlement_ids` entries are dropped.
