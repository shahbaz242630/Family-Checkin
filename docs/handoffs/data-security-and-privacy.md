...
| Data    | `apps/backend/prisma/schema.prisma`; `apps/backend/prisma/migrations/` (16); `apps/backend/prisma/supabase_setup.sql` + 4 hosted RLS patches; `supabase/migrations/20260510181345_partitioned_operational_logs.sql`; `apps/backend/scripts/db/sql/20260905_partition_rls_hardening.sql` |
...
Prisma models by domain (20 models, all snake_case `@@map`ped, all UUID PKs defaulted DB-side with `gen_random_uuid()`):

- People and consent — `User`, `Receiver`, `BackupContact`, `CoMonitor`.
- Check-in lifecycle — `CheckIn`, `CheckInAttempt`, `EscalationEvent`.
- Channels and provider plumbing — `ChannelTemplate`, `ProviderWebhookEvent`, `ProviderWebhookEventKey`, `VoiceCallerIdPool`, `ReceiverVoiceCallerIdAssignment`, `DeviceToken`, `ExpoPushTicket`.
...
- Code columns are `varchar`, never `char(n)`: `users.preferredLanguage`, `receivers.language` and `channel_templates.language` are `varchar(8)` since `202609060102_language_columns_varchar`. `char(5)` padded `"en"` to `"en   "` and Prisma returned it verbatim into API responses and lookup keys (CB-075). `country`/`countryCode` stay `char(2)` because ISO codes always fill the width.
- Seeded reference data lives in Prisma migrations with `ON CONFLICT … DO NOTHING` on the natural key (`202609060103_seed_channel_templates_8_languages`), so re-running `migrate deploy` or applying the file by hand on the hosted database is safe and never overwrites a corrected row.
...
- Pending on the hosted project (applied in the repo by #34, both idempotent): `202609060301_receiver_consent_resend_count` (`receivers.consentResendCount INTEGER NOT NULL DEFAULT 0`) and `202609060302_expo_push_tickets` (new table, index on `createdAt`, `ENABLE ROW LEVEL SECURITY`, no policies). After applying, confirm RLS is on for `expo_push_tickets` and that it has no policies; `check-invariants.mjs` registers it in `INTERNAL_ONLY_NO_POLICIES`. Docker was not available on the engineering machine for the local replay; the Database workflow ran the same checks on the PR.
...
- PRs: #18 (audit PII guard exempts `Id`/`Ids`/`Ref` keys, CB-002), #29 (CB-075 language columns to `varchar(8)`, CB-010 seed migration), #34 (CB-081 `receivers.consentResendCount`, CB-023 `expo_push_tickets`).
- Hosted database: everything through `202609060202` was applied on 2026-09-06; the two #34 migrations are the ones pending (see Hosted database above).
