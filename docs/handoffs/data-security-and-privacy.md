...
- Applied on the hosted project on 2026-09-07 (founder, management-API script; repo #34): `202609060301_receiver_consent_resend_count` (`receivers.consentResendCount INTEGER NOT NULL DEFAULT 0`) and `202609060302_expo_push_tickets` (new table, index on `createdAt`, `ENABLE ROW LEVEL SECURITY`, no policies). Verified after the apply: both present, `expo_push_tickets` RLS on with 0 policies, `receivers` still 3 policies; `check-invariants.mjs` registers the table in `INTERNAL_ONLY_NO_POLICIES`. The hosted schema is current through `202609060302`.
