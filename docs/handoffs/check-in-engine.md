...
Status: Built · Last verified: 2026-09-06 (acceptance run — S2/S6/S8; sender actions and voice caller-ID: specs; local-day dedupe, schedule-invalid stamp and `checkin_retry`: specs plus migration `202609060101` replayed on a throwaway Postgres with `db:apply-all`, `db:check-invariants` and `db:drift-check` green)
BRD: BRD-4.3, BRD-6.2, FR-CSC-01, FR-CSC-03, FR-CSC-04, FR-CSC-07, FR-REC-06 · Open backlog: CB-017, CB-045, CB-053, CB-057, CB-060, CB-069 (sender push and receiver-card state only, after CB-012)
...
- On each later tick, times out attempts whose response window has elapsed and sends the next attempt that is already due; a future attempt keeps the check-in open instead of ending it. Attempt 2 onwards is rendered from `checkin_retry` ("we have not heard back from you yet"); voice attempts keep `checkin_daily_voice` (CB-010).
- Stamps a receiver whose stored `timezone` or `scheduleTimeWindow` cannot be evaluated (`receivers.scheduleInvalidAt`) and audits `check_in.schedule_invalid` once per schedule version, not once per tick (CB-069).
...
- To see the once-only schedule audit, set a receiver's `timezone` to `Dubai` in the database and tick twice: one `check_in.schedule_invalid` audit row, `receivers.scheduleInvalidAt` set, `dueCheckIns.failed` 1 on both ticks. Restore the timezone and tick: the stamp is null again.
...
- `check_in.schedule_invalid` is written only when `markScheduleInvalid` flips `receivers.scheduleInvalidAt` from null (an `updateMany` guarded on `scheduleInvalidAt: null`, the same idiom as the status guards); the tick that sees the schedule evaluate again clears the stamp through `clearScheduleInvalid` so the next bad version is audited afresh (CB-069). Do not audit from the skipped list without the stamp.
...
- CB-069 — the audit is now once per schedule version, but the sender is never told and the receiver card does not show the invalid state; the quiet push waits for `NotificationsService.sendQuietUpdateToUser` (CB-012) and the card for the mobile slice.
