...
- Writes the whole channel cascade up front as `check_in_attempts` rows with staggered `scheduledAt`, claims attempt 1 (PENDING → SENT, `providerStatus: sending`) before the provider call, sends it, records the provider's id on the attempt and marks the check-in `SENT`.
- Runs each scheduler tick under a Postgres advisory lock: an overlapping `POST /operations/check-ins/run` answers `{ ok: true, locked: true }` at once and processes nothing, and every attempt is claimed atomically before it is sent, so two ticks can never send the same attempt (CB-045).
...
- A provider that rejects a send stamps the attempt with what actually went wrong: `TwilioRequestError` becomes `twilio_<code>` (for example `twilio_21606`) in `check_in_attempts.failureReason` and in the `check_in.attempt_failed` audit row (with `providerErrorCode`); anything else stays `provider_send_failed`. Every one of those failures is also written to the JSON log with the code and no PII (CB-047).
...
- `POST /operations/check-ins/run` — scheduler only. `Authorization: Bearer <OPERATIONS_CRON_SECRET>`, compared timing-safely by `assertBearerSecret` (`apps/backend/src/shared/auth/bearer-secret.ts`); missing or wrong bearer is `401 "Operations cron bearer token is required"`. Carries `@SkipThrottle()` because the scheduler calls it in bursts. Calls `CheckInsService.runScheduledTick()`, which takes the run lock and runs `sendDueCheckIns()` then `processCascadeAttempts()`, and returns aggregate counts only: `{ ok: true, dueCheckIns: { created, sent, skipped, failed }, cascadeAttempts: { sent, timedOut, failed, needsAttention, skipped } }` — no receiver ids, check-in ids, provider ids, names, phones or bodies. While another tick holds the lock the response is `200 { ok: true, locked: true }` with no counts, and `apps/backend/scripts/run-operations-check-ins.ts` (through `operations-runner.ts`) prints it as ok with zero counts, so the workflow stays green. `dueCheckIns.skipped` covers ineligible and unpaid receivers, a receiver whose local day an overlapping tick already claimed, and a first attempt an overlapping cascade pass claimed first; `cascadeAttempts.skipped` also counts attempts another tick claimed; `dueCheckIns.failed` counts a receiver with an unevaluable schedule on every tick even though it is audited once.
...
### Run lock and attempt claim (CB-045)

- `PrismaCheckInsRepository.runExclusively` opens a Prisma interactive transaction whose only statement is `SELECT pg_try_advisory_xact_lock(4045::int, 1::int)` (`CHECK_INS_RUN_LOCK_KEY`); when it returns false the tick resolves `{ locked: true }` without running. The work runs on the normal client, outside that transaction, which exists only to hold the lock and is released at commit or rollback — a crashed tick never leaves it held. `maxWait` is 5 s; `timeout` is `CHECK_INS_RUN_LOCK_TIMEOUT_MS` = 5 minutes, the same as `timeout-minutes` in `.github/workflows/operations-check-ins.yml`. A run that outlives the transaction loses the lock and then sees Prisma's `P2028` on commit; the repository returns the finished work's counts instead of failing the tick.
- The workflow's `concurrency: { group: operations-check-ins, cancel-in-progress: false }` already serialises GitHub-launched ticks; the database lock covers a manual `workflow_dispatch`, a second backend instance or an operator's curl overlapping the cron.
- The claim is `markAttemptSent` called *before* the provider: `updateMany({ where: { id, status: PENDING }, data: { status: SENT, sentAt, providerStatus: 'sending' } })`; `count === 0` means another tick has it and this one counts `skipped` and sends nothing. `recordAttemptSendResult` writes the provider's id and status afterwards, guarded on SENT. No new status or column was needed: an attempt that is SENT with `providerStatus: sending` and no `providerMessageId` is a tick that died between claim and provider answer; the timeout pass moves the cascade on after the response window exactly as for an unanswered message.
- `hasPendingAttempts` is `checkInAttempt.count({ checkInId, status: PENDING })`; `advanceCascade` fetches one row through `findNextDuePendingAttempt`; `findDuePendingAttempts` and `findTimedOutSentAttempts` take at most `ATTEMPT_SCAN_LIMIT` = 500 rows per tick, oldest first, so a backlog drains across ticks. Billing is looked up once per sender per tick (`sendDueCheckIns` memoises `getBillingStatus` by `userId`); a single-query batch would need a `BillingService` method that takes many user ids.
...
| attempt `markAttemptSent` → `SENT` (the cron's pre-send claim: `providerStatus: sending`, no provider id yet) | `PENDING` |
| attempt `recordAttemptSendResult` → provider id and status on the claimed attempt | `SENT` |
...
- To see the run lock, start two ticks at once (two PowerShell jobs or `curl` calls) against a database with a slow provider or many due rows: one returns the counts, the other `{ ok: true, locked: true }`; a lone tick never reports `locked`.
...
- An attempt is claimed (`markAttemptSent`, PENDING → SENT) *before* the provider is called and `false` means "do not send" (CB-045). Never move the claim after the send, never send from a scan result without it, and never widen the claim's `allowedFrom` beyond PENDING.
- The run-lock transaction holds the lock and nothing else: provider calls, audits and every other write run on the normal client outside it. Keep `runScheduledTick` the only entry point of the scheduler; a route that calls `sendDueCheckIns` / `processCascadeAttempts` directly runs unlocked.
- Both attempt scans are bounded (`ATTEMPT_SCAN_LIMIT`, oldest first). Do not remove the `take` or reorder the scans by anything but `scheduledAt`.
...
- The two-concurrent-ticks proof on a real Postgres (each attempt sent exactly once) belongs to CB-055's integration project; the lock and the claim are covered by specs with doubles (`check-ins.service.spec.ts`, `prisma-check-ins.repository.spec.ts`), and `pg_try_advisory_xact_lock` itself is exercised only when the backend runs against a database.
- `findReceiversDueForCheckIn` still loads every granted daily receiver (the window is evaluated in JS per receiver) and the dedupe lookup is one `OR` of every due receiver; both are unbounded and will need chunking well before ten thousand receivers.
...
- PRs: #20 (CB-004/005/006/008 — resilient tick, exhaustion notification, guarded transitions, cancellation), #21 (sprint 1 close: acceptance report and backlog follow-ups), #28 (CB-013 local-day dedupe, CB-069 once-per-version audit, CB-010 `checkin_retry`), #31 (CB-016 delivery-status failure advances the cascade, CB-069 sender quiet push, CB-010 sender display name in check-in copy), #35 (CB-045 advisory run lock, pre-send attempt claim, counted pending attempts, bounded scans, per-sender billing lookup). #39 (CB-047 — a Twilio rejection is recorded as `twilio_<code>` in the attempt and the audit row, and every swallowed failure in the tick is logged).
