...
- PRs: (#PR) (CB-042 step-up body schemas; CB-084 the deprecated read-path alias deleted and every call site renamed). None of the sprint-1 PRs recorded in the archive (#17–#20) touch this feature; the auth and account work predates them. #24 (CB-070: `StepUpService` exported to `ReceiversModule`; emulator acceptance report). #31 (CB-010: `users.displayNameEncrypted` stored by `sync-user`, `UsersService.senderDisplayNameFor`, export and deletion cover the name). #35 (CB-024: local JWT verification with `jose`, JWKS cache, `SUPABASE_JWT_SECRET`, read-only sender resolution outside `sync-user`, metadata fallbacks, 401/409/503 mapping).
