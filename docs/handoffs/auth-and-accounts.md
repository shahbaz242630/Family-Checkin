...
BRD: §5.1 FR-AUTH-01..04 · Open backlog: CB-025, CB-028, CB-033, CB-043, CB-044, CB-050 (CB-029 closed 2026-09-07)
...
- Every `familycheckin://` auth link is processed exactly once, by `useDeepLinks()` in the root layout (CB-029). It exchanges the code through `handleAuthDeepLink` and then re-renders `/auth/callback` or `/auth/reset-password` with `status` (and `message`) route params; neither screen calls `getInitialURL` any more. Signing up when the Supabase project requires email confirmation leaves no session, and the signup screen now says "Check your email" instead of pushing on to onboarding.
...
- Phone is never verified by OTP at signup (FR-AUTH-01 is unmet): it comes from `user_metadata.phone` written by the client. Biometric unlock no longer exists: `services/biometric.ts` and its settings toggle stored a preference nothing ever read and were deleted by CB-034 (2026-09-07).
...
| Mobile  | `apps/mobile/src/services/{supabase,auth,auth-storage}.ts`, `apps/mobile/src/contexts/AuthContext.tsx`, `apps/mobile/src/hooks/{useAuth,useProfile,useDeepLinks}.ts`, `apps/mobile/src/components/auth/ProtectedRoute.tsx`, `apps/mobile/src/app/{_layout.tsx,index.tsx,(auth)/,auth/,(main)/settings/}`          |
...
- `GET /auth/admin/me` — active admin only. Returns only `admin.id` and `admin.role`; no email, encrypted email or email hash. Inactive or unlisted → 403. The app calls it once per signed-in user id from `AuthContext` and caches the answer (a 403 included) to gate the admin drawer entries without asking again (CB-039).
- `POST /account/step-up/request` — authenticated sender. Body `{ action }` limited to `EXPORT_DATA | DELETE_ACCOUNT | REMOVE_RECEIVER` (checked by `stepUpRequestBodySchema` before the handler, then again by `parseSensitiveAction`; an unknown action is `400 VALIDATION_FAILED`, CB-042); sends a 6-digit OTP over SMS via the channel router; returns `challengeId`, `action`, `expiresAt` (10 min).
- `POST /account/step-up/verify` — authenticated sender. Body `{ challengeId, code }` (`stepUpVerifyBodySchema`: both required, the code 4–12 characters); returns `stepUpToken` and `expiresAt` (10 min). Wrong code increments the attempt counter; 5 attempts lock the challenge.
...
- Every authenticated route resolves the sender with `UsersService.findOrCreateFromSupabaseIdentity`. The `upsertFromSupabaseIdentity` alias is deleted (CB-084): only `POST /auth/sync-user` writes profile columns, through `syncProfileFromSupabaseIdentity`, and a second spelling of the read path must not come back.
...
- One deep-link handler, and it is `useDeepLinks()` in the root layout (CB-029). Nothing else may call `Linking.getInitialURL()`, subscribe to the `url` event, or call `handleAuthDeepLink`. A second call on the same link fails — the PKCE code has been exchanged and the SecureStore OAuth state consumed — which is exactly how a successful email confirmation used to flash "Verification Failed" and land the sender back on login. The handler keeps a set of URLs it has already handled because Android can deliver the launch intent through both `getInitialURL()` and the `url` event.
- `/auth/callback` and `/auth/reset-password` render from route params (`status`, `message`), never from the URL. A screen that reads the URL itself breaks the warm start: `getInitialURL()` returns the URL the app was *launched* with, not the link that has just arrived.
...
- CB-029 — done 2026-09-07 (#44): one handler in `hooks/useDeepLinks.ts`, both auth screens driven by route params, signup shows "Check your email" when `signUp` returns no session. Verified by `useDeepLinks.spec.ts`; the device pass (cold-start confirmation, warm-start recovery link) is still outstanding.
...
- PRs: #40 (CB-042 step-up body schemas; CB-084 the deprecated read-path alias deleted and every call site renamed). None of the sprint-1 PRs recorded in the archive (#17–#20) touch this feature; the auth and account work predates them. #24 (CB-070: `StepUpService` exported to `ReceiversModule`; emulator acceptance report). #31 (CB-010: `users.displayNameEncrypted` stored by `sync-user`, `UsersService.senderDisplayNameFor`, export and deletion cover the name). #35 (CB-024: local JWT verification with `jose`, JWKS cache, `SUPABASE_JWT_SECRET`, read-only sender resolution outside `sync-user`, metadata fallbacks, 401/409/503 mapping). #44 (CB-029: `hooks/useDeepLinks.ts` as the app's one deep-link handler, `/auth/callback` and `/auth/reset-password` driven by route params, the signup confirm-email state; CB-039: `AuthContext.isAdmin` from a cached `GET /auth/admin/me`. Four protected files edited under the founder's narrow approval of 2026-09-07).
