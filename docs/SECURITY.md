...
| Scheduled workflow `operations-check-ins.yml` is disabled by GitHub for inactivity | Re-enable once the backend is hosted and `OPERATIONS_CHECK_INS_RUN_URL` / `OPERATIONS_CRON_SECRET` are set. Its second step also needs `OPERATIONS_PUSH_RECEIPTS_RUN_URL` (the push-receipt route, CB-085); without it that step skips with a notice. |
